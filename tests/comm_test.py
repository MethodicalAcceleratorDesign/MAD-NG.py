...
class TestMatrices(unittest.TestCase):
    ...
    def test_send_recv_cmat(self):
	  ...
	  """)
            cmat = (
                np.arange(1, 25).reshape(4, 6) / 4
                + 1j * np.arange(1, 25).reshape(4, 6) / 4
            )
            mad.send(cmat)
...
class TestTPSA(unittest.TestCase):
    ...
    def test_send_tpsa(self):
	  ...
	  """)
            ...
            self.assertTrue(
                mad.recv("tab"),
                ["000", "100", "010", "001", "200", "110"].extend(coefficients),
            )  # intentional?
...
class TestOutput(unittest.TestCase):
    def test_print(self):
        with MAD() as mad:
            ...
            self.assertEqual(
                mad.recv(), "hello world"
            )  # Check printing does not affect pipe
