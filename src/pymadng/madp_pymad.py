...
class mad_process:
  def __init__(self, mad_path: Union[str, Path], py_name: str = "py", debug: Union[int, str, bool] = False) -> None:
    ...
    startup_status_checker = select.select([self.mad_read_stream], [], [], 10)  # May not work on windows
  ...
  def protected_send(self, string: str):
  ...
  def safe_recv_var(self, name: str):
  ...
  def set_error_handler(self, on_off: bool):
  ...
  def recv_vars(self, *names):
    if len(names) == 1:
      if not is_private(names[0]):
        return self.safe_recv_var(names[0])
    else:
      return tuple(self.safe_recv_var(name) for name in names if not is_private(name))
...
class mad_ref(object):
  ...
  def __getitem__(self, item: Union[str, int]):
    if isinstance(item, int):
      result = self._mad.safe_recv_var(f"{self._name}[{item+1}]")
      if result is None:
        raise IndexError(item)  # For python
    elif isinstance(item, str):
      result = self._mad.safe_recv_var(f"{self._name}['{item}']")
      if result is None:
...
recv_cpx_tpsa = lambda self: recv_generic_tpsa(self, np.dtype("complex128"))
recv_dbl_tpsa = lambda self: recv_generic_tpsa(self, np.dtype("float64"))
