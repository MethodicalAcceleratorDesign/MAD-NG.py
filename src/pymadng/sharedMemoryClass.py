...
class shmBuffer:
    ...
    def __setProperties(self, start: int, end: int) -> None:
        self.start = start
        self.end = end
        self.__bufferInfo[:] = np.array(
            [self.start, self.end, self.capacity], dtype=np.int32
        )[:]
    ...
    def read(self, dataSize: int) -> Any:
        ...
        # Change below to be bit manipulation
        newReadLocation = (
            ((self.start + dataSize) % (self.capacity)) // self.__PAGE_SIZE + 1
        ) * self.__PAGE_SIZE
        assert newReadLocation == (
            ((self.start + dataSize) // self.__PAGE_SIZE + 1) * self.__PAGE_SIZE
        ) % (self.capacity), "Houston, we have a problem"
