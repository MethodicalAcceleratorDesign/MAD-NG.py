...
# Builds MAD-NG third-party *static* libraries into: ./bin/macosx
#
# You still need system build tools. Typical (Homebrew) prerequisites:
...
sedi() {
  # Portable in-place sed across GNU/BSD
...
main() {
  ...
  prompt_default JOBS "Parallel jobs (make -j)" "$(sysctl -n hw.ncpu 2>/dev/null || echo 4)"
  prompt_default CC "C compiler (CC)" "clang"
  prompt_default CXX "C++ compiler (CXX)" "clang++"
